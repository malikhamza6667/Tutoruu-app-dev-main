...
export default function CounterScreen() {
  ...
  //     (state: RootState) => state.myApiReducer
  //   );
    ...
    // }
  const { loading, error, data } = useQuery<ClassesData, ClassesVars>(
    GET_CLASSES,
  );
  ...
  return (
    <View>
      <FlatList
        data={data?.classes}
        renderItem={({ item }) => <Text>{item._id}, {item.name}, {item.desc}, {item.courses_length} </Text>}
        keyExtractor={item => item._id}
      />
    </View>
  //   <View>
  //   <Button onPress={handleGetData} title="Get Data" />
  //   {data && <Text>{data}</Text>}
  // </View>
  );
}
